...
$("#formeditDiplomado").submit(function (event) {
    ...
    posting.done(function (data) {
        if (data !== null) {
            ...
            var fila;
            fila = '<td class="nombre_Diplomado">' + obj.NombreDiplomado + '</td>';
...
$("#frmDELdip").submit(function (event) {
    ...
    ;
    var postVer = $.post('DiplomadosController/VerificarDip', {CodigoDiplomado: CodigoDiplomado});  //Aqui muestra el error -------------------------------------->
    postVer.done(function (data) {
        if (data!=null) {
            if (data != 0) {
                var confir = confirm("Este Diplomado Contiene " + data + " Modulos,¿Aun desea  eliminarlo?");
                if (confir == true) {
                    var posting = $.post(url, {CodigoDiplomado: CodigoDiplomado});  //Aqui muestra el error -------------------------------------->
                    posting.done(function (dataDel) {
                        if (dataDel) {
                            var obj = jQuery.parseJSON(dataDel);
                            $("#EliminarDiplomado").modal('toggle');
    ...
    postVer.fail(function (xhr, textStatus, errorThrown) {
        alert("error" + xhr.responseText);
    });
